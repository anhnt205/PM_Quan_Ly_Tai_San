...
public class MoHinhTaiSanController {
    ...
    @GetMapping
    public List<MoHinhTaiSan> getAll(@RequestParam String idcongty) {
        return moHinhTaiSanService.getAll(idcongty);
    }

    @GetMapping("/paged")
    public PageResponse<MoHinhTaiSan> getAllPaged(
            @RequestParam String idcongty,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size,
            @RequestParam(required = false) String sortBy,
            @RequestParam(required = false) String sortDir) {
        return moHinhTaiSanService.getAllPaged(idcongty, page, size, sortBy, sortDir);
    }

    @GetMapping("/{id}")
    public MoHinhTaiSan getById(@PathVariable String id) {
    ...
    public ResponseEntity<ApiResponse<Object>> createBatch(@RequestBody List<MoHinhTaiSan> list) {
        try {
            int total = 0;
            for (MoHinhTaiSan item : list) {
                total += moHinhTaiSanService.create(item);
            }
            if (total > 0) {
                return ResponseEntity.status(HttpStatus.CREATED)
    ...
    public ResponseEntity<ApiResponse<Object>> deleteBatch(@RequestBody List<String> ids) {
        try {
            int total = 0;
            for (String id : ids) {
                total += moHinhTaiSanService.delete(id);
            }
            if (total > 0) {

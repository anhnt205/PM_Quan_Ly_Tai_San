...
public class DieuDongCCDCVatTuService {
    ...
    public PageResponse<DieuDongCCDCVatTuDTO> findAllPaged(String idCongTy, int page, int size, String sortBy,
        ...
        if (search != null && !search.trim().isEmpty()) {
            ...
            for (DieuDongCCDCVatTuDTO item : sourceList) {
                if (item != null && (loai == null || (item.getLoai() != null && item.getLoai().equals(loai)))
                        && item.toString() != null && item.toString().toLowerCase().contains(q)) {
                    if (item.getLoai().equals(loai)) {
                        item.setChiTietDieuDongCCDCVatTuDTOS(chiTietDieuDongCCDCVatTuDao.findAll(item.getId()));
                        filtered.add(item);
                    }
                }
            }
            ...
            List<DieuDongCCDCVatTuDTO> items = filtered.subList(from, to);
            ...
            for (DieuDongCCDCVatTuDTO item : filtered) {
                if (item != null) {

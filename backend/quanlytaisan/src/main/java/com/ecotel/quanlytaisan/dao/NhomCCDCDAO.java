...
public class NhomCCDCDAO {
    ...
    public List<NhomCCDC> findAllPaged(String idCongTy, int page, int size, String sortBy, String sortDir) {
        // Xử lý sortBy và sortDir
        String orderBy = "Id"; // default sort
        if (sortBy != null && !sortBy.trim().isEmpty()) {
            // Validate sortBy để tránh SQL injection
            String[] allowedColumns = {"Id", "Ten", "HieuLuc", "NgayTao", "NgayCapNhat"};
        ...
        String sql = "SELECT * FROM NhomCCDC WHERE IdCongTy=? ORDER BY " + orderBy + " " + direction + " LIMIT ? OFFSET ?";
        int offset = page * size;
        return jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(NhomCCDC.class), idCongTy, size, offset);
    }

...
public class LoaiTaiSanController {
    ...
    @GetMapping
    public List<LoaiTaiSan> getAll(@RequestParam String idcongty) {
        return loaiTaiSanService.getAll(idcongty);
    }

    @GetMapping("/paged")
    public PageResponse<LoaiTaiSan> getAllPaged(
            @RequestParam String idcongty,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size,
            @RequestParam(required = false) String sortBy,
            @RequestParam(required = false) String sortDir) {
        return loaiTaiSanService.getAllPaged(idcongty, page, size, sortBy, sortDir);
    }

    @GetMapping("/{id}")
    public LoaiTaiSan getById(@PathVariable String id) {
        return loaiTaiSanService.getById(id);
    }

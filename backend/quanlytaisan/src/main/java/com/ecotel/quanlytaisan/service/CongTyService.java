...
public class CongTyService {
    ...
    public PageResponse<CongTy> getAllPaged(int page, int size, String sortBy, String sortDir) {
        List<CongTy> items = congTyDao.findAllPaged(page, size, sortBy, sortDir);
        long totalItems = congTyDao.countAll();
    ...
    public List<CongTy> readCsv(MultipartFile file) throws IOException {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                CongTy ts = CongTy.mapToCongTy(fields); // map từ CSV sang object
                list.add(ts);
            }
        }
        return list;
    ...
    public List<CongTy> readExcel(MultipartFile file) throws IOException {
        ...
        for (Row row : sheet) {
            ...
            CongTy ts = CongTy.mapToCongTy(row); // map từ Row sang object
            list.add(ts);
        }
        workbook.close();
        return list;

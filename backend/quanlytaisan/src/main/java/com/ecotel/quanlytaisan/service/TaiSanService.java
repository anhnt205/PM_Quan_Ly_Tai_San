...
public class TaiSanService {
    ...
    public Map<String, Object> getAllPagedWithBanGiaoStatus(String idCongTy, int page, int size,
        ...
        for (TaiSanDTO item : all) {
            ...
            // Phan loai theo trang thai ban giao
    ...
    public PageResponse<TaiSanDTO> getPagedByBanGiaoStatus(String idCongTy, int page, int size,
        ...
        // Load danh sach kho thu hoi 1 lan thay vi query cho moi item
        java.util.Set<String> khoThuHoiIds = null;
        if (isBanGiao) {
            khoThuHoiIds = getKhoThuHoiIds(idCongTy);
        }

        List<TaiSanDTO> filtered = new ArrayList<>();
        for (TaiSanDTO item : all) {
            // Filter theo trang thai ban giao
            boolean daBanGiao = item.getIdDonViHienThoi() != null && !item.getIdDonViHienThoi().isEmpty();
            ...
            // Neu la danh sach da ban giao, chi lay tai san khong thuoc kho thu hoi
            if (isBanGiao && khoThuHoiIds != null && khoThuHoiIds.contains(item.getIdDonViHienThoi())) {
                continue;
            }
            // Neu co search, filter theo search
            if (q != null && !matchesTaiSanSearch(item, q)) {
                continue;
            }
            filtered.add(item);

...
public class DuAnController {
    ...
    @GetMapping
    public List<DuAn> getAll(@RequestParam String idcongty) {
        return duAnService.getAll(idcongty);
    }

    @GetMapping("/paged")
    public PageResponse<DuAn> getAllPaged(
            @RequestParam String idcongty,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size,
            @RequestParam(required = false) String sortBy,
            @RequestParam(required = false) String sortDir) {
        return duAnService.getAllPaged(idcongty, page, size, sortBy, sortDir);
    }

    @GetMapping("/{id}")
    public DuAn getById(@PathVariable String id) {
        return duAnService.getById(id);
    }

...
public class NhomTaiSanController {
    ...
    public ResponseEntity<ApiResponse<Object>> createBatch(@RequestBody List<NhomTaiSan> list) {
        try {
            int total = nhomTaiSanService.batchCreate(list);
            if (total > 0) {
                return ResponseEntity.status(HttpStatus.CREATED)
    ...
    @PutMapping("/batch")
    public ResponseEntity<ApiResponse<Object>> updateBatch(@RequestBody List<NhomTaiSan> list) {
        try {
            int total = nhomTaiSanService.batchUpdate(list);
            if (total > 0) {
                return ResponseEntity.ok(ApiResponse.success("Cập nhật danh sách nhóm tài sản thành công", null, total));
            }
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(ApiResponse.failure("Cập nhật danh sách nhóm tài sản thất bại", total));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.failure("Lỗi hệ thống: " + e.getMessage(), null));
        }
    ...
    public ResponseEntity<ApiResponse<Object>> deleteBatch(@RequestBody List<String> ids) {
        try {
            int total = nhomTaiSanService.batchDelete(ids);
            if (total > 0) {
    ...
    @Operation(summary = "Xoá toàn bộ nhóm tài sản")
    public ResponseEntity<?> deleteAll() {
        nhomTaiSanService.deleteAll();
        return ResponseEntity.ok("Đã xoá toàn bộ nhóm tài sản");
    }

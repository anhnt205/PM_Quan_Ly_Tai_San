...
import com.ecotel.quanlytaisan.service.BienPhapMayMocService;
...
@RestController
@RequestMapping("/api/bienphap-maymoc")
public class BienPhapMayMocController {

    @Autowired
    private BienPhapMayMocService service;
    ...
    public ResponseEntity<ApiResponse<Object>> getAllPaged(
        ...
        try {
            PageResponse<BienPhapMayMocDTO> response = service.findAllPaged(
    ...
    public ResponseEntity<ApiResponse<Object>> getAll(
        ...
        try {
            List<BienPhapMayMocDTO> list = service.findAll(idCongTy);
    ...
    public ResponseEntity<ApiResponse<Object>> getById(@PathVariable("id") String id) {
        try {
            BienPhapMayMocDTO dto = service.findByIdDTO(id);
            if (dto == null) return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(ApiResponse.failure("Không tìm thấy biên bản biện pháp máy móc", null));
    ...
    public ResponseEntity<ApiResponse<Object>> getPermissionSigning(
        ...
        try {
            BienPhapMayMocDTO item = service.findByIdDTO(id);
            if (item == null) return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(ApiResponse.failure(
                            "Không tìm thấy biên bản biện pháp máy móc với ID: " + id, null));

...
public class KeHoachSuaChua {
    ...
    public static KeHoachSuaChua mapToKeHoachSuaChua(String[] row) {
        ...
        kh.setIdDonViThucHien(safeGet(row, 6));
        kh.setIdNguoiPhuTrach(safeGet(row, 7));
        kh.setNgayBatDau(parseDate(safeGet(row, 8), "yyyy-MM-dd"));
        kh.setNgayKetThuc(parseDate(safeGet(row, 9), "yyyy-MM-dd"));
        kh.setLoaiDoiTuong(safeGet(row, 10));
    ...
    public static KeHoachSuaChua mapToKeHoachSuaChua(Row row) {
        ...
        kh.setIdDonViThucHien(getCellStringValue(row.getCell(6)));
        kh.setIdNguoiPhuTrach(getCellStringValue(row.getCell(7)));

        LocalDateTime ngayBD = getCellDate(row.getCell(8));
        kh.setNgayBatDau(ngayBD != null ? Date.from(ngayBD.atZone(ZoneId.systemDefault()).toInstant()) : null);

        LocalDateTime ngayKT = getCellDate(row.getCell(9));
        kh.setNgayKetThuc(ngayKT != null ? Date.from(ngayKT.atZone(ZoneId.systemDefault()).toInstant()) : null);

        kh.setLoaiDoiTuong(getCellStringValue(row.getCell(10)));

        LocalDateTime ngayTao = getCellDate(row.getCell(11));
        kh.setNgayTao(ngayTao != null ? Date.from(ngayTao.atZone(ZoneId.systemDefault()).toInstant()) : null);

        LocalDateTime ngayCapNhat = getCellDate(row.getCell(12));

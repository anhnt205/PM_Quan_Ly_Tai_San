...
import { CapitalSourceBulkValidation } from "../validation/Validation";
...
export interface CapitalSourceFormProps {
  /** "create" = thêm mới (hiện nút thêm/copy/xóa), "edit" = sửa (ẩn nút thêm/copy) */
  mode: "create" | "edit";
  initialItems?: CapitalSourceItem[];
  onSave: (items: CapitalSourceItem[]) => void;
  onCancel: () => void;
  onMinimize: () => void;
  onItemsChange?: (items: CapitalSourceItem[]) => void;
  initialFormData?: any;
}
...
const normalizeItem = (item: any): CapitalSourceItem => ({
  id: item?.id ?? "",
  tenNguonKinhPhi: item?.tenNguonKinhPhi ?? "",
  ghiChu: item?.ghiChu ?? "",
  idCongTy: item?.idCongTy ?? CongTy.CT001,
  hieuLuc: item?.hieuLuc ?? true,
...
export default function CapitalSourceForm({
  ...
  const formik = useFormik({
    ...
    validationSchema: CapitalSourceBulkValidation,
        ...
        <Box display="flex" alignItems="center" gap={1.5}>
          ...
          <Typography variant="h5" sx={{ fontWeight: 700, color: "#1FA463" }}>
            {isEdit && formik.values.items.length === 1
              ? "Chỉnh sửa nguồn vốn"
              : isEdit
                ? "Sửa hàng loạt nguồn vốn"
                : "Thêm mới nguồn vốn"}
          </Typography>
          <Chip
            label={`${formik.values.items.length} nguồn vốn`}
            size="small"
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid size={{ xs: 12, sm: 6 }}>
                        <FieldInput
                          ...
                          name={`items.${index}.tenNguonKinhPhi`}
                        />
                      </Grid>
                      <Grid size={{ xs: 12 }}>
                        <FieldInput

...
const RepairRequestDialog = ({
  open, onClose, plan, selectedDeviceIds, selectedMonth, onSubmit,
}: Props) => {
  ...
  const sourceDeptId = plan.sourceDepartmentId || '';
  const execDeptId = plan.executionDepartmentId || '';
  ...
  const handleRemoveSigner = (userId: string) => {
    setSigners(prev =>
      prev.filter(s => s.userId !== userId).map((s, i) => ({ ...s, order: i + 1 }))
    );
  };
  ...
  const handleSubmit = () => {
    ...
    const req: RepairRequest = {
      id: `GDN-${Date.now()}`,
      planId: plan.id,
      number: number || `${sourceDeptId}-${String(selectedMonth + 1).padStart(2, '0')}-${plan.year}`,
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth
      ...
      <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', pb: 1 }}>
        <Typography variant="h6" fontWeight={600}>Tạo Giấy đề nghị sửa chữa</Typography>
              ...
              <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 3, p: 2.5 }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    Căn cứ Kế hoạch SCBD tháng <b>{selectedMonth + 1}</b> năm <b>{plan.year}</b>
            ...
            <Box sx={{
              ...
              <Box sx={{
                ...
                <FormControl size="small" fullWidth disabled={!addDeptId}>
                  <InputLabel>Người duyệt</InputLabel>
                  <Select value={addUserId} label="Người duyệt"

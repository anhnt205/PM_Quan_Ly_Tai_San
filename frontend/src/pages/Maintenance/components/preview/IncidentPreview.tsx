...
import {
  ...
  TableHead, TableRow, Paper, Divider, TextField, Tooltip, IconButton,
  Select, MenuItem,
} from '@mui/material';
...
import type { DeviceEntryEditable } from '../dialog/IncidentDialog';
...
interface Props {
  ...
  description?: string;
  severity?: string;
  deviceEntries?: DeviceEntryEditable[];
  onDeviceEntriesChange?: (entries: DeviceEntryEditable[]) => void;
  planIds?: string[];
}

// ─── Ô text chỉnh sửa inline ───
const EditableCell = ({
  value, onChange, placeholder = '—', width = 100,
}: {
  ...
  if (editing) {
    return (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, minWidth: width }}>
        <TextField
          inputRef={inputRef} value={draft}
    ...
    <Tooltip title="Nhấn để chỉnh sửa" placement="top" arrow>
      <Box
        ...
        sx={{
          ...
          cursor: 'pointer', minWidth: width, borderRadius: 1, px: 0.5, py: 0.25,
          '&:hover': { bgcolor: 'action.hover' },
          '&:hover .edit-icon': { opacity: 1 },
        }}
      >
        <Typography sx={{ fontSize: 11, flex: 1, color: value ? 'text.primary' : 'text.disabled', fontStyle: value ? 'normal' : 'italic' }}>
          {value || placeholder}
        </Typography>
        <EditIcon className="edit-icon" sx={{ fontSize: 11, color: 'primary.main', opacity: 0, transition: 'opacity 0.15s', flexShrink: 0 }} />
      </Box>
    </Tooltip>
  );
};
...
const DeptSelectCell = ({
  value, onChange, departments, placeholder = '— Chọn đơn vị —', width = 140,
}: {
  value: string;
    ...
    <Tooltip title="Nhấn để chọn đơn vị" placement="top" arrow>
      <Box sx={{ minWidth: width }}>
        <Select
          ...
          renderValue={() => (
            <Typography sx={{ fontSize: 11, color: value ? 'text.primary' : 'text.disabled', fontStyle: value ? 'normal' : 'italic' }}>
              {value || placeholder}
            </Typography>
          )}
          sx={{
            ...
            '&:hover:not(.Mui-disabled):before': { borderBottom: '1px solid rgba(0,0,0,0.2)' },
            '& .MuiSelect-icon': { fontSize: 14, color: 'primary.main' },
            '& .MuiSelect-select': { py: 0.25, px: 0.5, borderRadius: 1, '&:hover': { bgcolor: 'action.hover' } },
          }}
          MenuProps={{ PaperProps: { sx: { maxHeight: 260 } } }}
        >
          <MenuItem value="" sx={{ fontSize: 11, fontStyle: 'italic', color: 'text.disabled' }}>
            — Chọn đơn vị —
          </MenuItem>
          {departments.map(d => (
            <MenuItem key={d.id} value={d.id} sx={{ fontSize: 11 }}>{d.name}</MenuItem>
          ))}
        </Select>
      </Box>
    </Tooltip>
...
const IncidentPreview = ({
  number, detectedAt, reporter, reporterDeptId,
  signers = [], systemName, subsystem, location,
  description, severity, deviceEntries = [],
  onDeviceEntriesChange, planIds = [],
}: Props) => {

  const { data: apiDepartments = [] } = useAllDepartmentsQuery();

  const departments: { id: string; name: string }[] = (apiDepartments || []).map((d: any) => ({
    id: String(d?.id ?? ''),
    name: String(d?.tenPhongBan ?? d?.name ?? ''),
  }));

  // Tên đơn vị báo cáo = đơn vị quản lý TB
  const reporterDeptName = departments.find(d => d.id === reporterDeptId)?.name ?? '';

  const today = new Date();
  const dateStr = detectedAt
    ? (() => { const d = new Date(detectedAt); return !isNaN(d.getTime()) ? `Quảng Ninh, ngày ${d.getDate()} tháng ${d.getMonth() + 1} năm ${d.getFullYear()}` : detectedAt; })()
    : `Quảng Ninh, ngày ${today.getDate()} tháng ${today.getMonth() + 1} năm ${today.getFullYear()}`;

  const detectedAtDisplay = detectedAt
    ? (() => { const d = new Date(detectedAt); return !isNaN(d.getTime()) ? d.toLocaleString('vi-VN', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit', year: 'numeric' }) : detectedAt; })()
    : '...';

  const updateEntry = (idx: number, field: keyof DeviceEntryEditable, value: string) => {
    if (!onDeviceEntriesChange) return;
    onDeviceEntriesChange(deviceEntries.map((e, i) => i === idx ? { ...e, [field]: value } : e));
  };
  ...
  return (
    <Paper variant="outlined" sx={{ p: 3, mt: 0, fontFamily: '"Times New Roman", serif' }}>
      ...
      <Typography sx={{ fontSize: 12, mb: 1, fontFamily: 'inherit' }}>- Đơn vị báo cáo: <b>{reporterDeptName || '...'}</b></Typography>
      <Typography sx={{ fontSize: 12, mb: 1, fontFamily: 'inherit' }}>- Ngày giờ phát hiện: <b>{detectedAtDisplay}</b></Typography>
      <Typography sx={{ fontSize: 12, mb: 1, fontFamily: 'inherit' }}>- Tên hệ thống/thiết bị gặp sự cố: <b>{systemName || '...'}</b></Typography>
      <Typography sx={{ fontSize: 12, mb: 1, fontFamily: 'inherit' }}>- Phân hệ/vị trí xảy ra sự cố: <b>{subsystem || '...'}</b></Typography>
      <Typography sx={{ fontSize: 12, mb: 1, fontFamily: 'inherit' }}>- Mô tả tình trạng sự cố: {description || '...'}</Typography>
      <Typography sx={{ fontSize: 12, mb: 2, fontFamily: 'inherit' }}>
        - Đánh giá mức độ: <b style={{ color: severity === 'Nghiêm trọng' ? '#d32f2f' : undefined }}>{severity || '...'}</b>
      </Typography>

      {/* Table */}
      <Typography sx={{ fontSize: 12, fontWeight: 700, mb: 1, fontFamily: 'inherit' }}>Danh sách hệ thống/thiết bị bị sự cố:</Typography>

      {deviceEntries.length > 0 && (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 1 }}>
          <EditIcon sx={{ fontSize: 12, color: 'primary.main' }} />
          <Typography variant="caption" color="primary.main">
            Các cột có biểu tượng bút / mũi tên có thể nhấn để chỉnh sửa trực tiếp
          </Typography>
        </Box>
      )}
      ...
      <TableContainer sx={{ mb: 2 }}>
        <Table size="small" sx={{ '& th, & td': { fontSize: 11, fontFamily: '"Times New Roman", serif', border: '1px solid #999', py: 0.5, px: 1 } }}>
            ...
            <TableRow sx={{ bgcolor: '#f9f9f9' }}>
              ...
              <TableCell align="center" sx={{ fontWeight: 700 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0.5 }}>
                  Thuộc hệ thống <EditIcon sx={{ fontSize: 11, color: 'primary.main' }} />
                </Box>
              </TableCell>
              <TableCell align="center" sx={{ fontWeight: 700 }}>Vị trí</TableCell>
              <TableCell align="center" sx={{ fontWeight: 700 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0.5 }}>
                  Tình trạng <EditIcon sx={{ fontSize: 11, color: 'primary.main' }} />
                </Box>
              </TableCell>
              ...
              <TableCell align="center" sx={{ fontWeight: 700 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0.5 }}>
                  Đơn vị quản lý KT <UnfoldMoreIcon sx={{ fontSize: 12, color: 'primary.main' }} />
                </Box>
              </TableCell>
          ...
          <TableBody>
            {deviceEntries.length > 0 ? deviceEntries.map((entry, idx) => (
              <TableRow key={`${entry.deviceId}-${idx}`}>
                ...
                <TableCell sx={{ minWidth: 110 }}>
                  <EditableCell value={entry.system || ''} onChange={v => updateEntry(idx, 'system', v)} placeholder="Nhập hệ thống..." width={100} />
                </TableCell>

                {/* Vị trí — từ API */}
                <TableCell align="center">{entry.position || '—'}</TableCell>

                {/* Tình trạng — map từ hienTrang, cho sửa */}
                <TableCell sx={{ minWidth: 120 }}>
                  <EditableCell value={entry.status || ''} onChange={v => updateEntry(idx, 'status', v)} placeholder="Nhập tình trạng..." width={110} />
                </TableCell>

                {/* Đơn vị quản lý TB — tự động = đơn vị báo cáo */}
                <TableCell align="center">{reporterDeptName || '—'}</TableCell>

                {/* Đơn vị quản lý KT — dropdown chọn phòng ban */}
                <TableCell sx={{ minWidth: 150 }}>
                  <DeptSelectCell
                    value={entry.technicalUnit || ''}
                    onChange={name => updateEntry(idx, 'technicalUnit', name)}
              ...
              <TableRow>
                <TableCell colSpan={11} align="center" sx={{ py: 2 }}>
                  <Typography variant="caption" color="text.secondary">Chưa có thiết bị nào được thêm</Typography>
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <Box sx={{ mt: 4, display: 'flex', justifyContent: 'space-between', gap: 2 }}>
        {(() => {
          ...
          if (cols.length === 0) {
            return <Box sx={{ flex: 1, textAlign: 'center' }}><Typography variant="caption" color="text.disabled">Chưa có người duyệt</Typography></Box>;
          }

...
import { UnitBulkValidation } from "../validation/Validation";
...
export interface UnitFormProps {
  ...
  onSave: (items: UnitItem[]) => void;
  onCancel: () => void;
  onMinimize: () => void;
  onItemsChange?: (items: UnitItem[]) => void;
  initialFormData?: any;
}
...
const normalizeItem = (item: any): UnitItem => ({
  id: item?.id ?? "",
...
export default function UnitForm({
  ...
  const formik = useFormik({
    ...
    validationSchema: UnitBulkValidation,
        ...
        <Box display="flex" alignItems="center" gap={1.5}>
          ...
          <Typography variant="h5" sx={{ fontWeight: 700, color: "#1FA463" }}>
            {isEdit && formik.values.items.length === 1
              ...
              : isEdit
                ? "Sửa hàng loạt đơn vị tính"
                : "Thêm mới đơn vị tính"}
          </Typography>
          <Chip
            label={`${formik.values.items.length} đơn vị`}
            size="small"
                    ...
                    >
                      <Box display="flex" alignItems="center" gap={1}>
                        ...
                        <Typography sx={{ fontWeight: 600, color: "#1FA463" }}>
                          {item.tenDonVi || item.id || `Đơn vị tính ${index + 1}`}
                        </Typography>
                      </Box>
                      <Box display="flex" gap={0.5}>

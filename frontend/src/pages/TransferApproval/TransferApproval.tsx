...
import { getAssetTransferCount, getToolTransferCount, findById } from "../../utils/helpers";
...
export default function TransferApproval() {
  ...
  const assetCount1 = getAssetTransferCount(1, user?.taiKhoan?.tenDangNhap, assetTransfer.items, isBanHanh);
  const assetCount2 = getAssetTransferCount(2, user?.taiKhoan?.tenDangNhap, assetTransfer.items, isBanHanh);
  const assetCount3 = getAssetTransferCount(3, user?.taiKhoan?.tenDangNhap, assetTransfer.items, isBanHanh);
  const totalAssetCount = assetCount1 + assetCount2 + assetCount3;

  const { data: toolTransfer = { items: [] } } = useToolTransferPageQuery(0, 999999);
  const toolCount1 = getToolTransferCount(1, user?.taiKhoan?.tenDangNhap, toolTransfer.items, isBanHanh);
  const toolCount2 = getToolTransferCount(2, user?.taiKhoan?.tenDangNhap, toolTransfer.items, isBanHanh);
  const toolCount3 = getToolTransferCount(3, user?.taiKhoan?.tenDangNhap, toolTransfer.items, isBanHanh);

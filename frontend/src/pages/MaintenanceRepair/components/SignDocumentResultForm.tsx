...
interface SignDocumentFormResultProps {
  selectedIds: string[];
...
export default function SignDocumentResultForm({
  selectedIds,
  ...
  useEffect(() => {
    const rebuildBangKe = async () => {
      try {
        const bytes = await generateBangKeKetQuaPdf(
          data,
          allUnits,
          allCurrentStatus,
        );
    ...
    rebuildBangKe();
  }, [data, allUnits, allCurrentStatus]);

 ...
 */
export function mapPortalPermissionsToRoles(permissions: Record<string, any>): Array<{ permissionCode: string }> {
  const roles: Array<{ permissionCode: string }> = [];
  if (!permissions) return roles;

  const keys = Object.keys(permissions);
  const hasKey = (pattern: string) => keys.some((k) => k.toLowerCase().includes(pattern));

  if (hasKey("nhan-vien") || hasKey("nhanvien")) roles.push({ permissionCode: "NHANVIEN" });
  if (hasKey("phong-ban") || hasKey("phongban")) roles.push({ permissionCode: "PHONGBAN" });
  if (hasKey("du-an") || hasKey("duan")) roles.push({ permissionCode: "DUAN" });
  if (hasKey("nguon-von") || hasKey("nguonvon") || hasKey("nguon-kinh-phi")) roles.push({ permissionCode: "NGUONVON" });
  
  // Tài sản
  if (hasKey("tai-san") || hasKey("taisan")) {
    roles.push({ permissionCode: "TAISAN" });
  }
  if (hasKey("ban-giao-tai-san") || hasKey("bangiaotaisan") || hasKey("ban-giao")) {
    roles.push({ permissionCode: "BANGIAO_TAISAN" });
  }
  if (hasKey("dieu-dong-tai-san") || hasKey("dieudongtaisan") || hasKey("dieu-dong")) {
  ...
  if (hasKey("bao-cao") || hasKey("baocao")) roles.push({ permissionCode: "BAOCAO" });

  ...
  return (
    ...
    >
      ...
      >
        ...
        <Typography variant="h6" fontWeight={700} sx={{ flex: 1, color: "#111827" }}>
          {view === "list" ? "Danh sách kết nối CSDL" : currentConfig.id ? "Sửa cấu hình kết nối" : "Thêm mới cấu hình kết nối"}
        </Typography>
      </DialogTitle>
              ...
              <List sx={{ py: 1 }}>
                {configs.map((item: any, index: number) => (
                  <React.Fragment key={item.id}>
                        ...
                        <Box display="flex" gap={0.5}>
                          <Tooltip title="Thử kết nối">
                            <IconButton 
                              ...
                              color="info" 
                              size="small"
                              sx={{ "&:hover": { bgcolor: alpha("#0288d1", 0.1) } }}
                            >
                    ...
                    {index < configs.length - 1 && <Divider component="li" sx={{ mx: 3 }} />}
                  </React.Fragment>
                ))}
              </List>
            )}
